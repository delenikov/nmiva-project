...
Use this guide when a task touches `frontend/`, React UI, routing, PWA/service worker behavior,
offline IndexedDB data, sync queue behavior, push subscription UI, or frontend-facing backend contracts.

## Loading

- This file applies to all work under `frontend/`.
- Follow the root `../AGENTS.md` repo-wide rules together with this frontend-specific guide.
- This project uses Vite and React, not Next.js. Treat the React guidance below as supporting guidance and
  ignore Next.js-specific advice unless it clearly applies.

## Project Snapshot

- React 19, TypeScript, Vite 7, React Router 7, Tailwind CSS 4.
- PWA support uses `vite-plugin-pwa` with an injected service worker at `frontend/src/sw.ts`.
- Runtime app state lives in `frontend/src/context/AppContext.tsx`.
- Backend API calls live in `frontend/src/services/api.ts`.
- Auth token storage lives in `frontend/src/services/authStore.ts`.
- Offline data and sync live in `frontend/src/services/localDb.ts`, `syncCore.ts`,
  `syncWorkerClient.ts`, and `frontend/src/workers/syncWorker.ts`.
- Shared API/model types live in `frontend/src/types/`.
- Reusable UI primitives live in `frontend/src/components/ui/`.

## Working Rules

- Preserve offline-first behavior. User writes should continue to update IndexedDB first and enqueue sync work.
- Keep frontend API types aligned with backend DTOs and sync responses.
- Do not break client/server ID reconciliation, `lastModifiedAt` conflict behavior, or queued mutation processing.
- Keep auth boundaries explicit. Protected screens should require a token and handle logout/expired auth cleanly.
- Do not log tokens, passwords, push subscriptions, or other sensitive payloads.
- Prefer existing UI primitives from `frontend/src/components/ui/` before adding new component styles.
- Use `lucide-react` icons for common actions when an icon is appropriate.
- Keep mobile-first PWA behavior intact, including installability, service worker registration, and offline fallback.
- Avoid broad visual rewrites unless the task is explicitly about redesign.
...
- API request and response types still match backend contracts.
- Offline writes, queue processing, and worker messages still work.
- Routes remain protected where required.
- PWA/service worker changes do not break build-time injection.
- Text and controls fit on mobile and desktop widths.
- New UI follows existing component and Tailwind patterns.
